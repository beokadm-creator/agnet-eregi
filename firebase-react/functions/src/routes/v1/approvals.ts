...
import { requireAuth, requireApprover, requireOps, roleOf } from "../../lib/auth";
import { fail, ok } from "../../lib/http";
import { withIdempotency } from "../../lib/idempotency";
import { approvalRef, decideApproval } from "../../lib/approvals";
...
export function registerApprovalRoutes(app: express.Express, adminApp: typeof admin) {
  // ops: approvals list
  ...
  app.post("/v1/ops/approvals/:approvalId/decision", async (req, res) => {
    const auth = await requireAuth(adminApp, req, res);
    if (!auth) return;
    if (!requireApprover(auth)) return fail(res, 403, "FORBIDDEN", "승인 권한이 없습니다.");

    const approvalId = req.params.approvalId;
    const { decision, reasonKo } = req.body ?? {};
    if (decision !== "approve" && decision !== "reject") return fail(res, 400, "INVALID_ARGUMENT", "decision이 필요합니다.");

    const result = await withIdempotency(adminApp, req, res, "approvals.decision", async () => {
      ...
      if (target?.caseId) {
        ...
        await writeTimelineEvent(adminApp, target.caseId, eventId, {
      ...
      if (decision === "approve" && a.gate === "quote_finalize") {
        ...
        if (t?.caseId && t?.quoteId) {
          ...
          const eventId = crypto.randomUUID();
          await writeTimelineEvent(adminApp, t.caseId, eventId, {
