...
const ops_alert_1 = require("./ops_alert");
    ...
    for (const doc of snap.docs) {
        ...
        if (success) {
            ...
            if (newAttempts >= maxAttempts) {
                ...
                const errorInfo = (0, ops_audit_1.categorizeError)(errorMessage);
                // (옵션) 슬랙 알림 발송 (notifyOpsAlert 공통 함수 사용) - 중복 방지
                if (!job.alertSentAt) {
                    await (0, ops_alert_1.notifyOpsAlert)({
                ...
                // Dead Letter 자동 이슈화
                try {
                    const issueResult = await (0, ops_actions_1.createDeadLetterIssueAction)(adminApp, job.gateKey, job.action, errorMessage, newAttempts, jobId, job.sourceEventId);
                    ...
                    else if (issueResult === null || issueResult === void 0 ? void 0 : issueResult.skipped) {
