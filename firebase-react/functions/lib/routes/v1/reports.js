...
function registerReportRoutes(app, adminApp) {
    ...
    app.get("/v1/cases/:caseId/reports/closing.docx", async (req, res) => {
        ...
        const refunds = refundSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
        const children = [];
        ...
        children.push(new docx_1.Paragraph({ text: `casePackId: ${c.casePackId ?? "-"}` }));
        children.push(new docx_1.Paragraph({ text: `status: ${c.status ?? "-"}` }));
        children.push(new docx_1.Paragraph({ text: `ownerUid: ${c.ownerUid ?? "-"}` }));
        children.push(new docx_1.Paragraph({ text: `partnerId: ${c.partnerId ?? "-"}` }));
        ...
        children.push(new docx_1.Paragraph({ text: `stage: ${wf?.stage ?? "-"}` }));
        children.push(new docx_1.Paragraph({ text: "" }));
        ...
        if (!filing) {
            ...
            children.push(new docx_1.Paragraph({ text: `receiptNo: ${filing.receiptNo ?? "-"}` }));
            children.push(new docx_1.Paragraph({ text: `jurisdictionKo: ${filing.jurisdictionKo ?? "-"}` }));
            children.push(new docx_1.Paragraph({ text: `submittedDate: ${filing.submittedDate ?? "-"}` }));
            if (filing.memoKo)
