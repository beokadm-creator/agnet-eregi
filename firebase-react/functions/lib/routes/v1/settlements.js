...
const settlement_status_1 = require("../../lib/settlement_status");
const payout_1 = require("../../lib/payout");
function settlementRef(adminApp, settlementId) {
    return adminApp.firestore().doc(`settlements/${settlementId}`);
}
function settlementItemRef(adminApp, settlementId, itemId) {
    return adminApp.firestore().doc(`settlements/${settlementId}/items/${itemId}`);
}
function settlementPayoutRef(adminApp, settlementId, payoutId) {
    return adminApp.firestore().doc(`settlements/${settlementId}/payouts/${payoutId}`);
}
function receivableRef(adminApp, partnerId, rid) {
    return adminApp.firestore().doc(`partners/${partnerId}/receivables/${rid}`);
}
function receivableOffsetRef(adminApp, partnerId, rid, offsetId) {
...
function registerSettlementRoutes(app, adminApp) {
    ...
    app.get("/v1/partner/settlements/:settlementId/items", async (req, res) => {
        ...
        if (!sSnap.exists)
            return (0, http_1.fail)(res, 404, "NOT_FOUND", "정산을 찾을 수 없습니다.");
        const s = sSnap.data();
        if (s.partnerId !== pid)
    ...
    app.get("/v1/partner/receivables/:receivableId/offsets", async (req, res) => {
        ...
        if (!rSnap.exists)
            return (0, http_1.fail)(res, 404, "NOT_FOUND", "미수금을 찾을 수 없습니다.");
        const snap = await adminApp
            .firestore()
    ...
    app.get("/v1/ops/settlements", async (req, res) => {
        ...
        let q = adminApp.firestore().collection("settlements");
        if (status)
    ...
    app.post("/v1/ops/settlements/generate", async (req, res) => {
        ...
        if (!partnerId || !periodFrom || !periodTo)
            return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "partnerId/periodFrom/periodTo가 필요합니다.");
        const result = await (0, idempotency_1.withIdempotency)(adminApp, req, res, "settlements.generate", async () => {
            ...
            const carryOverPrev = Number((carrySnap.exists ? carrySnap.data().carryOverAmount?.amount : 0) ?? 0);
            ...
            const recSnap = await adminApp
                ...
                .limit(50)
                .get();
            const receivables = recSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
            ...
            for (const r of receivables) {
                ...
                recBatch.set(receivableRef(adminApp, String(partnerId), r.id), {
                ...
                const offId = node_crypto_1.default.randomUUID();
            ...
            for (const p of payments) {
                const itemId = node_crypto_1.default.randomUUID();
    ...
    async function buildSettlementCsv(settlementId) {
        const sSnap = await settlementRef(adminApp, settlementId).get();
        if (!sSnap.exists)
    ...
    app.get("/v1/ops/settlements/:settlementId/export.csv", async (req, res) => {
        ...
        try {
            ...
            if (String(e?.message) === "NOT_FOUND")
                return (0, http_1.fail)(res, 404, "NOT_FOUND", "정산을 찾을 수 없습니다.");
            return (0, http_1.fail)(res, 500, "INTERNAL", "서버 오류가 발생했습니다.");
        }
    });
    ...
    app.post("/v1/ops/settlements/:settlementId/pay", async (req, res) => {
        ...
        if (!(0, auth_1.requireOps)(auth))
            return (0, http_1.fail)(res, 403, "FORBIDDEN", "운영자 권한이 없습니다.");
        const settlementId = req.params.settlementId;
        const result = await (0, idempotency_1.withIdempotency)(adminApp, req, res, "settlements.pay", async () => {
            const now = adminApp.firestore.FieldValue.serverTimestamp();
            const sSnap = await settlementRef(adminApp, settlementId).get();
            if (!sSnap.exists)
                throw new Error("NOT_FOUND");
            const s = sSnap.data();
            const from = String(s.status || "created");
            if (from === "paid")
                return { settlementId, status: "paid" };
            if (!(0, settlement_status_1.isAllowedSettlementTransition)(from, "paid"))
                throw new Error("INVALID_TRANSITION");
            const payoutAmount = Number(s.amountPayout?.amount ?? 0);
            const currency = String(s.currency ?? "KRW");
            if (payoutAmount <= 0)
                throw new Error("INVALID_ARGUMENT:지급 금액이 0입니다(이월 상태).");
            // 1) payout attempt 생성
            const payoutId = node_crypto_1.default.randomUUID();
            await settlementPayoutRef(adminApp, settlementId, payoutId).set({
                ...
                requestedBy: { uid: auth.uid, role: (0, auth_1.roleOf)(auth) },
                createdAt: now,
                updatedAt: now
            });
            // 2) 지급 실행(외부 연동 포인트)
            const pr = await (0, payout_1.executePayout)({
                settlementId,
            ...
            if (pr.ok) {
                await adminApp.firestore().runTransaction(async (t) => {
                    ...
                    if (!snap2.exists)
                        throw new Error("NOT_FOUND");
                    const s2 = snap2.data();
                    const from2 = String(s2.status || "created");
                    if (from2 !== "paid" && !(0, settlement_status_1.isAllowedSettlementTransition)(from2, "paid"))
                        throw new Error("INVALID_TRANSITION");
                    t.set(ref, {
                        status: "paid",
                        paidAt: now,
                        updatedAt: now,
                        paidBy: { uid: auth.uid, role: (0, auth_1.roleOf)(auth) },
                        lastPayoutId: payoutId
                    }, { merge: true });
                    t.set(settlementPayoutRef(adminApp, settlementId, payoutId), { status: "succeeded", provider: pr.provider, providerRef: pr.providerRef, updatedAt: now }, { merge: true });
                });
                await adminApp.firestore().collection("auditLogs").add({
                    type: "SETTLEMENT_PAID",
                    settlementId,
                    partnerId: s.partnerId,
                    actor: { uid: auth.uid, role: (0, auth_1.roleOf)(auth) },
                    payoutId,
                ...
                await adminApp.firestore().runTransaction(async (t) => {
                    ...
                    const s2 = snap2.data();
                    const from2 = String(s2.status || "created");
                    if (from2 !== "failed" && !(0, settlement_status_1.isAllowedSettlementTransition)(from2, "failed"))
                        throw new Error("INVALID_TRANSITION");
                    t.set(ref, {
                        status: "failed",
                        updatedAt: now,
                    ...
                    t.set(settlementPayoutRef(adminApp, settlementId, payoutId), { status: "failed", provider: pr.provider, error: pr.error, updatedAt: now }, { merge: true });
                });
                await adminApp.firestore().collection("auditLogs").add({
            ...
            if (msg === "NOT_FOUND") {
                (0, http_1.fail)(res, 404, "NOT_FOUND", "정산을 찾을 수 없습니다.");
                return null;
            }
