...
function PartnerArtboard() {
  return (
    <div className="ar-root ar-paper" style={{ width: 1440, minHeight: 1000, display: 'flex' }}>
      {/* Sidebar */}
      <aside style={{ width: 240, background: 'var(--ar-canvas)', borderRight: '1px solid var(--ar-hairline)', padding: '20px 14px', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="ar-card" style={{ padding: 12, margin: '4px 0 12px', display: 'flex', alignItems: 'center', gap: 10, background: 'var(--ar-paper-alt)', border: 'none' }}>
          <window.Avatar name="해" size={32} hue={50} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>해담 법무사</div>
            <div style={{ fontSize: 11, color: 'var(--ar-slate)' }}>강남 사무소 · 김재원</div>
          </div>
          <window.Ic.chev style={{ color: 'var(--ar-slate)' }} />
        </div>

        <div className="ar-nav-section">메인</div>
        <NavItem active icon={<window.Ic.layers />} label="케이스 워크보드" badge="12" />
        <NavItem icon={<window.Ic.inbox />} label="신규 의뢰" badge="3" badgeAccent />
        <NavItem icon={<window.Ic.doc />} label="서류함" />
        <NavItem icon={<window.Ic.bolt />} label="견적 · 청구" />

        <div className="ar-nav-section">설정</div>
        <NavItem icon={<window.Ic.user />} label="팀원" />
        <NavItem icon={<window.Ic.shield />} label="템플릿" />
        <NavItem icon={<window.Ic.card />} label="정산" />
        <NavItem icon={<window.Ic.bell />} label="알림 설정" />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 32px', borderBottom: '1px solid var(--ar-hairline)' }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <div style={{ position: 'relative' }}>
              ...
              <window.Ic.search style={{ position: 'absolute', left: 10, top: 10, color: 'var(--ar-slate)' }} />
            </div>
            <button className="ar-btn ar-btn-sm ar-btn-ghost"><window.Ic.filter /> 필터</button>
            <button className="ar-btn ar-btn-sm ar-btn-ink"><window.Ic.plus /> 사건 등록</button>
          </div>
        </div>

        {/* Stat row */}
        <div style={{ padding: '20px 32px 0', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          <window.StatTile label="신규 의뢰 (오늘)" value="3건" delta={50} hint="평균 응답 18분" />
          <window.StatTile label="서명 대기" value="5건" hint="고객 응답 대기 중" accent />
          <window.StatTile label="이번 주 매출" value="₩4.2M" delta={8} />
          <window.StatTile label="평균 처리 시간" value="5.4h" delta={-12} hint="목표 6시간" />
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 4px' }}>
        ...
        <button className="ar-btn ar-btn-quiet" style={{ height: 24, padding: '0 6px', fontSize: 11 }}><window.Ic.plus /></button>
      </div>
...
function CaseCard({ title, client, amount, age, tags = [], progress, flag, status, priority }) {
  return (
    <div className="ar-card" style={{ padding: 14, position: 'relative' }}>
      ...
      {flag && (
        <div style={{ marginTop: 10, padding: '6px 10px', background: 'var(--ar-warning-soft)', borderRadius: 6, fontSize: 11, color: 'oklch(40% 0.13 75)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
          <window.Ic.alert /> {flag}
        </div>
      )}
      {progress != null && (
...
window.PartnerArtboard = PartnerArtboard;
