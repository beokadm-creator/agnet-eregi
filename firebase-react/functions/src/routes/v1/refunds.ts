...
import { executeRefund } from "../../lib/refund_executor";
...
export function registerRefundRoutes(app: express.Application, adminApp: typeof admin) {
  ...
  app.post("/v1/ops/cases/:caseId/refunds/:refundId/execute", async (req: express.Request, res: express.Response) => {
    try {
      ...
      try {
        await executeRefund(db, refundId, opsId);
        return ok(res, { message: "환불이 실행되었습니다.", status: "executed" });
      } catch (execErr: any) {
        const msg = execErr.message;
        let statusCode = 500;
        let codeStr = "INTERNAL";
        
        if (msg.startsWith("NOT_FOUND:")) {
          statusCode = 404; codeStr = "NOT_FOUND";
        } else if (msg.startsWith("FAILED_PRECONDITION:")) {
          statusCode = 400; codeStr = "FAILED_PRECONDITION";
        } else if (msg.startsWith("INVALID_ARGUMENT:")) {
          statusCode = 400; codeStr = "INVALID_ARGUMENT";
        }
        
        return fail(res, statusCode, codeStr as any, msg);
      }
    } catch (err: any) {
