...
export async function processPartnerRankings(adminApp: typeof admin) {
  ...
  // 1. 모든 활성 파트너 조회
  const snapshot = await db.collection("partners").where("status", "==", "active").get();
  
  if (snapshot.empty) return;

  const partners = snapshot.docs.map(doc => ({ id: doc.id, ref: doc.ref, data: doc.data() }));

  // 2. 상대 평가를 위한 최대값 도출 (가격, 처리시간, 리뷰 수)
  ...
  for (const partner of partners) {
    ...
    // 현재 파트너의 진행 중인 케이스 수 조회 (Capacity 계산용)
    const activeCasesSnap = await db.collection("cases")
      .where("partnerId", "==", partner.id)
      .where("status", "in", ["draft", "collecting", "packaging"])
      .get();
    const activeCaseCount = activeCasesSnap.size;
