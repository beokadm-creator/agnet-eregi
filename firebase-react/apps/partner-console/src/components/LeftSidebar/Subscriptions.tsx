...
export default function Subscriptions() {
  ...
  return (
    <div style={{ borderTop: "2px solid var(--ar-surface-muted)", paddingTop: 16, marginTop: 16 }}>
      <h3 style={{ margin: "0 0 12px 0", color: "var(--ar-accent)", fontSize: "1.1em" }}>플랜 구독 및 멤버십 (EP-07-03)</h3>
      
      <div style={{ display: "flex", gap: 16, alignItems: "flex-start", flexWrap: "wrap" }}>
        {/* 현재 구독 정보 */}
        <div style={{ flex: 1, minWidth: 300, background: "var(--ar-accent-soft)", padding: 16, borderRadius: "var(--ar-r1)", border: "1px solid var(--ar-accent-soft)" }}>
          <h4 style={{ margin: "0 0 12px 0", color: "var(--ar-ink)" }}>내 구독 상태</h4>
          {subscription ? (
            <div>
              <div style={{ fontWeight: "bold", fontSize: "1.1em", marginBottom: 4 }}>
                플랜 ID: {subscription.planId} <span style={{ color: subscription.status === "active" ? "var(--ar-success)" : "var(--ar-danger)", fontSize: "0.8em" }}>({subscription.status})</span>
              </div>
              <div style={{ color: "var(--ar-graphite)", fontSize: "0.9em", marginBottom: 12 }}>
              ...
              {!subscription.cancelAtPeriodEnd && subscription.status === "active" && (
                <Button onClick={cancelSubscription} disabled={busy} style={{ background: "var(--ar-danger)", color: "var(--ar-canvas)", border: "none", padding: "6px 12px", borderRadius: "var(--ar-r1)", cursor: "pointer", fontSize: "0.85em" }}>
                  구독 해지 예약
                </Button>
              )}
            </div>
          ) : (
            <div style={{ color: "var(--ar-graphite)", fontSize: "0.9em" }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {subscriptionPlans.map(plan => (
              <div key={plan.id} style={{ background: "var(--ar-canvas)", padding: 16, borderRadius: "var(--ar-r1)", border: "1px solid var(--ar-hairline)", flex: 1, minWidth: 200 }}>
                ...
                <ul style={{ margin: "0 0 12px 0", paddingLeft: 20, fontSize: "0.85em", color: "var(--ar-graphite)" }}>
                ...
                <Button 
                  ...
                  style={{ width: "100%", padding: 8, background: (subscription && subscription.planId === plan.id) ? "var(--ar-slate)" : "var(--ar-accent)", color: "var(--ar-canvas)", border: "none", borderRadius: "var(--ar-r1)", cursor: "pointer", fontWeight: "bold" }}
                >
