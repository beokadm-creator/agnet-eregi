...
function registerReportRoutes(app, adminApp) {
    ...
    app.post("/v1/ops/reports/:gateKey/daily/append", async (req, res) => {
        ...
        try {
            ...
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey: req.params.gateKey,
                action: "daily.append",
                status: "fail",
                actorUid: ((_c = req.user) === null || _c === void 0 ? void 0 : _c.uid) || "unknown", // req.user may not exist if requireAuth fails earlier, but usually this is caught before
    ...
    app.post("/v1/ops/reports/:gateKey/audit/retry", async (req, res) => {
        try {
            ...
            if (eventData.status === "success") {
                return (0, http_1.fail)(res, 400, "FAILED_PRECONDITION", "이미 성공한 이벤트입니다.");
            }
            if (!(0, ops_retry_1.isRetryableAction)(eventData.action)) {
                return (0, http_1.fail)(res, 400, "FAILED_PRECONDITION", `Action '${eventData.action}'은(는) 재시도할 수 없습니다.`);
            }
    ...
    app.post("/v1/ops/reports/:gateKey/backlog/issues/project/add", async (req, res) => {
        ...
        try {
            ...
            for (const issue of issues) {
                ...
                try {
                    ...
                    if (!dryRun) {
                        ...
                        if (!issueRes.ok)
                            throw new Error(`Issue fetch failed: ${issueRes.status}`);
            ...
            `;
                        ...
                        if (addData.errors)
                            throw new Error(`GraphQL Error (add): ${JSON.stringify(addData.errors)}`);
