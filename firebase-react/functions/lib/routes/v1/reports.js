...
const ops_audit_1 = require("../../lib/ops_audit");
const ops_retry_1 = require("../../lib/ops_retry");
const ops_actions_1 = require("../../lib/ops_actions");
const ops_alert_1 = require("../../lib/ops_alert");
const ops_circuit_breaker_1 = require("../../lib/ops_circuit_breaker");
const ops_gate_settings_1 = require("../../lib/ops_gate_settings");
function fmtTs(v) {
    if (!v)
        return "-";
    try {
...
function registerReportRoutes(app, adminApp) {
    // Gate 일일 집계록(MD) 조회 API (운영용)
    app.get("/v1/ops/reports/:gateKey/daily.md", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            }
            const auth = await (0, auth_1.requireAuth)(adminApp, req, res);
            if (!auth)
                return;
            if (!(0, auth_1.isOps)(auth)) {
                (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/daily.md", code: "FORBIDDEN", messageKo: "운영자만 접근 가능합니다." });
            ...
            if (isNaN(targetDate.getTime())) {
                (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/daily.md", code: "INVALID_ARGUMENT", messageKo: "날짜 형식이 잘못되었습니다." });
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "날짜 형식이 잘못되었습니다. YYYY-MM-DD");
            }
                ...
                if (d.ok === false || d.status === "fail") {
                    ...
                    if (Array.isArray(d.missing)) {
                        for (const m of d.missing) {
                            ...
                            if (!missingStats[m])
                                missingStats[m] = { impactCount: 0, evidenceIds: new Set(), sampleCaseIds: new Set() };
            ...
            const items = Object.entries(missingStats).map(([slotId, stats]) => {
                ...
                if (slotId === "slot_filing_receipt")
                    sevNum = 1;
                else if (slotId.endsWith("_signed"))
                    sevNum = 2;
                return {
                    slotId,
                    severity: `Sev${sevNum}`,
            ...
            (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/daily.md", code: "INTERNAL", messageKo: "일일 로그 생성 중 시스템 오류가 발생했습니다.", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "일일 로그 생성 중 시스템 오류가 발생했습니다.");
        }
    });
    // Gate 일일 집계 API (운영용)
    app.get("/v1/ops/reports/:gateKey/daily", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!auth)
                return;
            if (!(0, auth_1.isOps)(auth)) {
                (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/daily", code: "FORBIDDEN", messageKo: "운영자만 접근 가능합니다." });
                return (0, http_1.fail)(res, 403, "FORBIDDEN", "운영자만 접근 가능합니다.");
            }
            const dateParam = String(req.query.date || "");
            const targetDateStr = dateParam ? dateParam : formatKstYmd();
            const targetDate = new Date(targetDateStr);
            if (isNaN(targetDate.getTime())) {
                (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/daily", code: "INVALID_ARGUMENT", messageKo: "날짜 형식이 잘못되었습니다." });
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "날짜 형식이 잘못되었습니다. YYYY-MM-DD");
            }
            const startOfDay = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), 0, 0, 0, 0);
            const endOfDay = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), 23, 59, 59, 999);
            ...
            (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/daily", code: "INTERNAL", messageKo: "집계 중 시스템 오류가 발생했습니다.", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "집계 중 시스템 오류가 발생했습니다.");
        }
    });
    // 일일 로그 SSOT 단일 조회
    app.get("/v1/ops/reports/:gateKey/daily/ssot", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (isNaN(targetDate.getTime())) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 날짜입니다.");
            }
            ...
            if (!logDoc.exists) {
                return (0, http_1.fail)(res, 404, "NOT_FOUND", "해당 날짜의 SSOT 로그가 없습니다.");
            }
            return res.status(200).json({ ok: true, data: logDoc.data() });
        }
        catch (e) {
            console.error("[daily ssot get error]", e);
            return (0, http_1.fail)(res, 500, "INTERNAL", e.message);
        }
    });
    // 최근 SSOT 목록 조회
    app.get("/v1/ops/reports/:gateKey/daily/ssot/recent", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (days < 1 || days > 30) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "days는 1~30 사이여야 합니다.");
            }
    ...
    app.post("/v1/ops/reports/:gateKey/daily/append", async (req, res) => {
        var _a, _b;
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
            ...
            if (date) {
                ...
                if (!/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
                    return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "날짜 형식이 잘못되었습니다. YYYY-MM-DD 포맷을 사용해주세요.");
                }
                // 실존 날짜 검증 (round-trip)
                ...
                if (dateStr !== roundTripStr) {
                    return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "존재하지 않는 날짜입니다.");
                }
            ...
            const requestId = req.headers["x-request-id"] || ((_a = req.body) === null || _a === void 0 ? void 0 : _a._requestId) || "N/A";
            ...
            try {
                ...
                const isAlreadyExists = e.code === 6 ||
                    e.status === "ALREADY_EXISTS" ||
                    (e.message && e.message.includes("ALREADY_EXISTS")) ||
                    (e.details && e.details.includes("ALREADY_EXISTS"));
                if (isAlreadyExists) {
                    (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/daily/append", code: "CONFLICT", messageKo: "해당 날짜의 로그가 이미 존재합니다.", err: e });
                    return (0, http_1.fail)(res, 409, "CONFLICT", "해당 날짜의 로그가 이미 존재합니다.");
                }
                throw e;
            }
            const linesAdded = markdown.split("\n").length + 2; // +2 for separator visual logic if needed
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey,
                action: "daily.append",
                status: "success",
                actorUid: auth.uid,
            ...
            const requestId = req.headers["x-request-id"] || ((_b = req.body) === null || _b === void 0 ? void 0 : _b._requestId) || "N/A";
            (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/daily/append", code: "INTERNAL", messageKo: "일일 로그 저장 중 시스템 오류가 발생했습니다.", err });
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
            ...
            return (0, http_1.fail)(res, 500, "INTERNAL", "일일 로그 저장 중 시스템 오류가 발생했습니다.");
        }
    });
    // 월간 리포트 생성 API (Ops-only)
    app.post("/v1/ops/reports/:gateKey/monthly/generate", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            const result = await (0, ops_actions_1.generateMonthlyReportAction)(adminApp, gateKey, targetMonth, dryRun);
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey,
            ...
            (0, http_1.logError)({ endpoint: "/monthly/generate", code: "INTERNAL", messageKo: "월간 리포트 생성 중 오류가 발생했습니다.", err });
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey: String(req.params.gateKey),
                action: "monthly.generate",
                status: "fail",
            ...
            return (0, http_1.fail)(res, 500, "INTERNAL", "월간 리포트 생성 중 오류가 발생했습니다.");
        }
    });
    // 월간 리포트 조회 API (Ops-only)
    app.get("/v1/ops/reports/:gateKey/monthly", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!/^\d{4}-\d{2}$/.test(month)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "month 파라미터 형식이 잘못되었습니다 (YYYY-MM).");
            }
            const docRef = adminApp.firestore().collection("ops_monthly_reports").doc(`${gateKey}:${month}`);
            const doc = await docRef.get();
            if (!doc.exists) {
                return (0, http_1.fail)(res, 404, "NOT_FOUND", "월간 리포트가 없습니다. 먼저 Generate API를 실행하세요.");
            }
            return res.status(200).json({ ok: true, data: doc.data() });
        }
        catch (err) {
            (0, http_1.logError)({ endpoint: "/monthly", code: "INTERNAL", messageKo: "월간 리포트 조회 중 오류가 발생했습니다.", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "월간 리포트 조회 중 오류가 발생했습니다.");
        }
    });
    // 월간 리포트 PR 조회 API (Ops-only)
    app.get("/v1/ops/reports/:gateKey/monthly/pr", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!configDoc.exists) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "GitHub 설정이 없습니다. 먼저 설정을 완료하세요.");
            }
            ...
            if (!GITHUB_TOKEN) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", `GitHub 토큰이 설정되지 않았습니다. (tokenRef: ${tokenRef})`);
            }
            ...
            if (!ghRes.ok) {
                ...
                return (0, http_1.fail)(res, 500, "INTERNAL", `GitHub PR 조회 실패: ${ghRes.status} ${errText}`);
            }
            const prList = await ghRes.json();
            if (prList.length === 0) {
            ...
            (0, http_1.logError)({ endpoint: "/monthly/pr", code: "INTERNAL", messageKo: "PR 조회 중 오류가 발생했습니다.", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "PR 조회 중 오류가 발생했습니다.");
        }
    });
    // 월간 리포트 Workflow Run 상태 조회 API (Ops-only)
    app.get("/v1/ops/reports/:gateKey/monthly/workflow-run", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!ghRes.ok) {
                const errText = await ghRes.text();
                return (0, http_1.fail)(res, 500, "INTERNAL", `GitHub Workflow Run 조회 실패: ${ghRes.status} ${errText}`);
            }
            const runData = await ghRes.json();
            const expectedName = `monthly-summary ${gateKey} ${month}`;
            let run = (runData.workflow_runs || []).find((r) => r.name === expectedName || r.display_title === expectedName);
            if (!run) {
                // 하위 호환성을 위해 브랜치명으로도 폴백 검색
                const headBranch = `bot/ops-monthly-summary-${gateKey}-${month}`;
                run = (runData.workflow_runs || []).find((r) => r.head_branch === headBranch);
            }
            if (!run) {
                return res.status(200).json({ ok: true, data: { exists: false } });
            }
            ...
            (0, http_1.logError)({ endpoint: "/monthly/workflow-run", code: "INTERNAL", messageKo: "Workflow Run 조회 중 오류가 발생했습니다.", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "Workflow Run 조회 중 오류가 발생했습니다.");
        }
    });
    // 월간 리포트 Workflow Run Dispatch API (Ops-only)
    app.post("/v1/ops/reports/:gateKey/monthly/workflow-run/dispatch", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            const result = await (0, ops_actions_1.dispatchWorkflowAction)(adminApp, gateKey, month, gateKeyOverride);
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
            ...
            (0, http_1.logError)({ endpoint: "/monthly/workflow-run/dispatch", code: "INTERNAL", messageKo: "Workflow Dispatch 중 오류가 발생했습니다.", err });
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey: String(req.params.gateKey),
                action: "workflow.dispatch",
                status: "fail",
            ...
            return (0, http_1.fail)(res, 500, "INTERNAL", "Workflow Dispatch 중 오류가 발생했습니다.");
        }
    });
    // 백로그 후보 자동화: Firestore SSOT → GitHub Issue 생성
    app.post("/v1/ops/reports/:gateKey/backlog/issues/create", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            const logDoc = await logDocRef.get();
            if (!logDoc.exists) {
                return (0, http_1.fail)(res, 404, "NOT_FOUND", "먼저 SSOT 저장을 수행하세요.");
            }
            const configDoc = await adminApp.firestore().collection("ops_github_project_config").doc(gateKey).get();
            const config = configDoc.data() || {};
            const githubConfig = config.github || {};
...
3. ok: true 달성
          `.trim();
                    ...
                    if (!dryRun) {
                        ...
                        if (!ghRes.ok) {
                            ...
                            const { category } = (0, ops_audit_1.categorizeError)(errorMsg);
                            await (0, ops_circuit_breaker_1.recordCircuitBreakerFail)(adminApp, gateKey, category, errorMsg);
            ...
            (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/backlog/issues/create", code: "INTERNAL", messageKo: "이슈 생성 중 오류가 발생했습니다.", err });
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey: String(req.params.gateKey),
                action: "issue.create",
                status: "fail",
            ...
            return (0, http_1.fail)(res, 500, "INTERNAL", "이슈 생성 중 오류가 발생했습니다.");
        }
    });
    // 백로그 자동화: 설정 SSOT 갱신 (Discovery)
    app.post("/v1/ops/reports/:gateKey/backlog/project/discover", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            const configData = await (0, ops_actions_1.discoverProjectConfigAction)(adminApp, gateKey, req.body.projectId, auth.uid);
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey,
            ...
            (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/backlog/project/discover", code: "INTERNAL", messageKo: "설정 갱신 중 오류가 발생했습니다.", err });
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey: String(req.params.gateKey),
                action: "project.discover",
                status: "fail",
            ...
            return (0, http_1.fail)(res, 500, "INTERNAL", "설정 갱신 중 오류가 발생했습니다.");
        }
    });
    // 백로그 자동화: gateKey별 GitHub 기본 설정 upsert (Ops-only)
    app.patch("/v1/ops/reports/:gateKey/backlog/project/config/github", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (owner !== undefined || repo !== undefined || projectId !== undefined || tokenRef !== undefined || tokenRefActions !== undefined) {
                updateData.github = {};
                if (owner !== undefined)
                    updateData.github.owner = owner;
                if (repo !== undefined)
                    updateData.github.repo = repo;
                if (projectId !== undefined)
                    updateData.github.projectId = projectId;
                if (tokenRef !== undefined) {
                    if (typeof tokenRef !== "string") {
                        return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "tokenRef는 문자열이어야 합니다.");
                    }
                    updateData.github.tokenRef = tokenRef;
                }
                if (tokenRefActions !== undefined) {
                    if (typeof tokenRefActions !== "string") {
                        return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "tokenRefActions는 문자열이어야 합니다.");
                    }
            ...
            if (issueLabels !== undefined || rules !== undefined) {
                ...
                if (issueLabels !== undefined)
                    updateData.rules.issueLabels = issueLabels;
            }
            ...
            return (0, http_1.fail)(res, 500, "INTERNAL", err.message);
        }
    });
    // 백로그 자동화: 설정 SSOT 조회
    app.get("/v1/ops/reports/:gateKey/backlog/project/config", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!doc.exists) {
                return (0, http_1.fail)(res, 404, "NOT_FOUND", "Project 설정이 없습니다. 먼저 Discover API를 실행하세요.");
            }
            return res.status(200).json({ ok: true, data: doc.data() });
        }
        catch (err) {
    ...
    app.patch("/v1/ops/reports/:gateKey/backlog/project/config/aliases", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!customAliases || typeof customAliases !== "object") {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "customAliases 객체가 필요합니다.");
            }
            ...
            const config = (await docRef.get()).data();
            const result = (0, ops_actions_1.doResolve)(config.rawFields || [], config.customAliases || {});
            await docRef.update({
            ...
            return res.status(200).json({ ok: true, data: Object.assign(Object.assign({}, config), result) });
        }
        catch (err) {
            (0, http_1.logError)({ endpoint: "aliases patch", code: "INTERNAL", messageKo: "Alias 업데이트 실패", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "Alias 업데이트 실패");
        }
    });
    // 백로그 자동화: 로컬 재매칭 (Re-resolve)
    app.post("/v1/ops/reports/:gateKey/backlog/project/resolve", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!(0, auth_1.isOps)(auth))
                return (0, http_1.fail)(res, 403, "FORBIDDEN", "운영자만 접근 가능합니다.");
            const result = await (0, ops_actions_1.doResolveAction)(adminApp, gateKey, auth.uid);
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey,
            ...
            (0, http_1.logError)({ endpoint: "resolve post", code: "INTERNAL", messageKo: "Re-resolve 실패", err });
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey: String(req.params.gateKey),
                action: "project.resolve",
                status: "fail",
            ...
            return (0, http_1.fail)(res, 500, "INTERNAL", "Re-resolve 실패");
        }
    });
    // 통합 감사 이벤트 목록 조회 (필터 및 커서 페이지네이션)
    app.get("/v1/ops/audit-events", async (req, res) => {
        try {
            const auth = await (0, auth_1.requireAuth)(adminApp, req, res);
            if (!auth)
                return;
            ...
            if (gateKey)
                query = query.where("gateKey", "==", String(gateKey));
            if (action)
                query = query.where("action", "==", String(action));
            if (status)
                query = query.where("status", "==", String(status));
            if (actorUid)
                query = query.where("actorUid", "==", String(actorUid));
            let startDate;
            let endDate;
            if (from && to) {
            ...
            if (cursor) {
                try {
                    ...
                    return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "잘못된 cursor 포맷입니다.");
                }
            }
            ...
            const items = snap.docs.map(doc => {
                const data = doc.data();
                return Object.assign(Object.assign({ id: doc.id }, data), { createdAt: fmtTs(data.createdAt) });
            });
            let nextCursor = null;
            ...
            (0, http_1.logError)({ endpoint: "audit-events", code: "INTERNAL", messageKo: "감사 이벤트 목록 조회 실패", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "감사 이벤트 목록 조회 실패: " + err.message);
        }
    });
    // 감사 이벤트 단건 상세 조회
    app.get("/v1/ops/audit-events/:id", async (req, res) => {
        try {
            const auth = await (0, auth_1.requireAuth)(adminApp, req, res);
            if (!auth)
                return;
            ...
            if (!doc.exists) {
                return (0, http_1.fail)(res, 404, "NOT_FOUND", "해당 이벤트를 찾을 수 없습니다.");
            }
            const data = doc.data();
            const item = Object.assign(Object.assign({ id: doc.id }, data), { createdAt: fmtTs(data === null || data === void 0 ? void 0 : data.createdAt) });
            return res.status(200).json({ ok: true, data: { item } });
        }
        catch (err) {
            (0, http_1.logError)({ endpoint: "audit-events/detail", code: "INTERNAL", messageKo: "감사 이벤트 상세 조회 실패", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "감사 이벤트 상세 조회 실패: " + err.message);
        }
    });
    // 최근 감사 이벤트 조회 (하위 호환)
    app.get("/v1/ops/reports/:gateKey/audit/recent", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            (0, http_1.logError)({ endpoint: "audit/recent", code: "INTERNAL", messageKo: "이벤트 조회 실패", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "이벤트 조회 실패");
        }
    });
    app.get("/v1/ops/audit/recent", async (req, res) => {
        try {
            const auth = await (0, auth_1.requireAuth)(adminApp, req, res);
            if (!auth)
                return;
            ...
            (0, http_1.logError)({ endpoint: "audit/recent/all", code: "INTERNAL", messageKo: "이벤트 조회 실패", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "이벤트 조회 실패");
        }
    });
    // 재시도 API (Ops-only)
    app.post("/v1/ops/reports/:gateKey/audit/retry", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!eventId || typeof eventId !== "string") {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "eventId 파라미터가 필요합니다.");
            }
            ...
            if (eventData.status === "success") {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "이미 성공한 이벤트입니다.");
            }
            if (!(0, ops_retry_1.isRetryableAction)(eventData.action)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", `Action '${eventData.action}'은(는) 재시도할 수 없습니다.`);
            }
            const { jobId, status } = await (0, ops_retry_1.enqueueRetryJob)(adminApp, eventId, eventData);
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey,
                action: "audit.retry",
                status: "success",
            ...
            (0, http_1.logError)({ endpoint: "audit/retry", code: "INTERNAL", messageKo: "재시도 생성 실패", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "재시도 생성 실패: " + err.message);
        }
    });
    // 재시도 현황 조회
    app.get("/v1/ops/retry/recent", async (req, res) => {
        try {
            const auth = await (0, auth_1.requireAuth)(adminApp, req, res);
            if (!auth)
                return;
            if (!(0, auth_1.isOps)(auth))
                return (0, http_1.fail)(res, 403, "FORBIDDEN", "운영자만 접근 가능합니다.");
            const limit = Number(req.query.limit) || 50;
            const gateKey = req.query.gateKey;
            ...
            (0, http_1.logError)({ endpoint: "retry/recent", code: "INTERNAL", messageKo: "재시도 현황 조회 실패", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "재시도 현황 조회 실패");
        }
    });
    // 1) GET /v1/ops/reports/:gateKey/circuit-breaker
    app.get("/v1/ops/reports/:gateKey/circuit-breaker", async (req, res) => {
        try {
            ...
            if (!(0, auth_1.isOps)(auth))
                return (0, http_1.fail)(res, 403, "FORBIDDEN", "운영자만 접근 가능합니다.");
            const state = await (0, ops_circuit_breaker_1.getCircuitBreakerState)(adminApp, gateKey);
    ...
    app.post("/v1/ops/reports/:gateKey/circuit-breaker/reset", async (req, res) => {
        try {
            const gateKey = String(req.params.gateKey);
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!(0, auth_1.isOps)(auth))
                return (0, http_1.fail)(res, 403, "FORBIDDEN", "운영자만 접근 가능합니다.");
            await (0, ops_circuit_breaker_1.resetCircuitBreaker)(adminApp, gateKey, `Manual reset by ${auth.uid}`);
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey,
    ...
    app.get("/v1/ops/gates/:gateKey/settings", async (req, res) => {
        try {
            const auth = await (0, auth_1.requireAuth)(adminApp, req, res);
            if (!auth)
                return;
            ...
            const gateKey = String(req.params.gateKey);
            if (!gateKey)
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "gateKey is required");
            const docRef = (0, ops_gate_settings_1.getOpsGateSettingsCollection)().doc(gateKey);
            const docSnap = await docRef.get();
            if (docSnap.exists) {
                return res.status(200).json({ ok: true, data: docSnap.data() });
            }
            else {
    ...
    app.get("/v1/ops/gates/:gateKey/settings/history", async (req, res) => {
        try {
            const auth = await (0, auth_1.requireAuth)(adminApp, req, res);
            if (!auth)
                return;
            ...
            (0, http_1.logError)({ endpoint: "gate-settings-history", code: "INTERNAL", messageKo: "Gate 설정 이력 조회 실패", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "Gate 설정 이력 조회 실패: " + err.message);
        }
    });
    // PUT /v1/ops/gates/:gateKey/settings
    app.put("/v1/ops/gates/:gateKey/settings", async (req, res) => {
        var _a;
        try {
            const auth = await (0, auth_1.requireAuth)(adminApp, req, res);
            if (!auth)
            ...
            if (slackWebhookUrl && typeof slackWebhookUrl === "string") {
                try {
                    ...
                    if (url.protocol !== "https:")
                        return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "slackWebhookUrl must be https");
                    if (!url.host)
                        return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "slackWebhookUrl must have a host");
                }
                catch (_b) {
                    return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "Invalid slackWebhookUrl format");
                }
            }
            const docRef = (0, ops_gate_settings_1.getOpsGateSettingsCollection)().doc(gateKey);
            const docSnap = await docRef.get();
            ...
            // Audit Log
            const oldHost = (oldData === null || oldData === void 0 ? void 0 : oldData.slackWebhookUrl) ? new URL(oldData.slackWebhookUrl).host : null;
            ...
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                ...
                target: {
                    gateKey,
                    changed: {
                        enabled: [(_a = oldData === null || oldData === void 0 ? void 0 : oldData.enabled) !== null && _a !== void 0 ? _a : true, enabled],
                        slackWebhookUrlHost: [oldHost, newHost],
                        notes: [(oldData === null || oldData === void 0 ? void 0 : oldData.notes) || null, notes || null]
                    }
                }
            });
            return res.status(200).json({ ok: true, data: Object.assign(Object.assign({}, oldData), newData) });
        }
        catch (err) {
            return (0, http_1.fail)(res, 500, "INTERNAL", err.message);
        }
    ...
    app.post("/v1/ops/alerts/test", async (req, res) => {
        try {
            const auth = await (0, auth_1.requireAuth)(adminApp, req, res);
            if (!auth)
                return;
            ...
            if (!gateKey)
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "gateKey is required");
            const alertResult = await (0, ops_alert_1.notifyOpsAlert)({
                gateKey,
            ...
            if (!(alertResult === null || alertResult === void 0 ? void 0 : alertResult.success) && (alertResult === null || alertResult === void 0 ? void 0 : alertResult.reason) === "No webhook URL configured") {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "Webhook URL이 설정되지 않았습니다.");
            }
            if (!(alertResult === null || alertResult === void 0 ? void 0 : alertResult.sent) && (alertResult === null || alertResult === void 0 ? void 0 : alertResult.reason) === "disabled") {
                return res.status(200).json({
                    ok: true,
                    data: {
                        sent: false,
            ...
            await (0, ops_audit_1.logOpsEvent)(adminApp, Object.assign({ gateKey, action: "ops_alert.test", status: (alertResult === null || alertResult === void 0 ? void 0 : alertResult.success) ? "success" : "fail", actorUid: auth.uid, requestId: req.headers["x-request-id"] || "N/A", summary: `알림 발송 테스트 (${alertResult === null || alertResult === void 0 ? void 0 : alertResult.resolvedFrom})`, target: {
                    resolvedFrom: (alertResult === null || alertResult === void 0 ? void 0 : alertResult.resolvedFrom) || "none",
                    webhookHost: (alertResult === null || alertResult === void 0 ? void 0 : alertResult.webhookHost) || null
                } }, ((alertResult === null || alertResult === void 0 ? void 0 : alertResult.success) ? {} : { error: { message: (alertResult === null || alertResult === void 0 ? void 0 : alertResult.reason) || "알림 발송 실패" } })));
            if (!(alertResult === null || alertResult === void 0 ? void 0 : alertResult.success)) {
                return (0, http_1.fail)(res, 500, "INTERNAL", `Webhook 전송 실패: ${alertResult === null || alertResult === void 0 ? void 0 : alertResult.reason}`);
            }
            return res.status(200).json({
                ok: true,
                data: {
    ...
    app.post("/v1/ops/retry/:jobId/deadletter/issue", async (req, res) => {
        var _a;
        try {
            ...
            if (!jobDoc.exists)
                return (0, http_1.fail)(res, 404, "NOT_FOUND", "Job not found");
            const job = jobDoc.data();
            if (job.status !== "dead") {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "Only dead jobs can trigger dead-letter issue creation.");
            }
            const errorMessage = ((_a = job.lastError) === null || _a === void 0 ? void 0 : _a.message) || "Unknown error";
            const { createDeadLetterIssueAction } = await Promise.resolve().then(() => __importStar(require("../../lib/ops_actions")));
    ...
    app.post("/v1/ops/reports/:gateKey/backlog/issues/project/add", async (req, res) => {
        var _a, _b;
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
            ...
            if (snap.empty) {
                ...
                const legacySnap = await adminApp.firestore()
                    ...
                    .get();
                if (legacySnap.empty) {
                    return (0, http_1.fail)(res, 404, "NOT_FOUND", "해당 날짜에 생성된 GitHub 이슈가 없습니다. (먼저 이슈를 생성하세요)");
                }
                ...
                issues = validDocs.map(d => (Object.assign({ dedupeKey: d.id }, d.data())));
            }
            else {
                issues = snap.docs.map(d => (Object.assign({ dedupeKey: d.id }, d.data())));
            }
            const configDoc = await adminApp.firestore().collection("ops_github_project_config").doc(gateKey).get();
            if (!configDoc.exists) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "Project 설정이 없습니다. 먼저 Discover API를 실행하세요.");
            }
            const config = configDoc.data();
            const githubConfig = config.github || {};
            ...
            if (!owner || !repo || !projectId || !GITHUB_TOKEN) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "GitHub 연동 설정이나 토큰이 누락되었습니다.");
            }
            await (0, ops_circuit_breaker_1.checkCircuitBreaker)(adminApp, gateKey);
            const added = [];
            const skipped = [];
            ...
            `;
                        ...
                        const statusOptionId = (_a = resolved.statusOptionIds) === null || _a === void 0 ? void 0 : _a[statusValue];
                        const priorityOptionId = (_b = resolved.priorityOptionIds) === null || _b === void 0 ? void 0 : _b[priorityValue];
                        const missingForThisIssue = [];
                        if (!STATUS_FIELD_ID)
                            missingForThisIssue.push("statusFieldId");
                        if (!statusOptionId)
                            missingForThisIssue.push(`status.${statusValue}`);
                        if (!PRIORITY_FIELD_ID)
                            missingForThisIssue.push("priorityFieldId");
                        if (!priorityOptionId)
                            missingForThisIssue.push(`priority.${priorityValue}`);
            ...
            (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/backlog/issues/project/add", code: "INTERNAL", messageKo: "프로젝트 투입 중 오류가 발생했습니다.", err });
            await (0, ops_audit_1.logOpsEvent)(adminApp, {
                gateKey: String(req.params.gateKey),
                action: "project.add",
                status: "fail",
            ...
            return (0, http_1.fail)(res, 500, "INTERNAL", "프로젝트 투입 중 오류가 발생했습니다.");
        }
    });
    // Gate 백로그 후보 생성 API (운영용)
    app.post("/v1/ops/reports/:gateKey/backlog", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!auth)
                return;
            if (!(0, auth_1.isOps)(auth)) {
                (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/backlog", code: "FORBIDDEN", messageKo: "운영자만 접근 가능합니다." });
                return (0, http_1.fail)(res, 403, "FORBIDDEN", "운영자만 접근 가능합니다.");
            }
            ...
            const targetDate = new Date(targetDateStr);
            if (isNaN(targetDate.getTime())) {
                (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/backlog", code: "INVALID_ARGUMENT", messageKo: "날짜 형식이 잘못되었습니다." });
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "날짜 형식이 잘못되었습니다. YYYY-MM-DD");
            }
            const startOfDay = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), 0, 0, 0, 0);
            const endOfDay = new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate(), 23, 59, 59, 999);
            ...
            (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/backlog", code: "INTERNAL", messageKo: "백로그 생성 중 시스템 오류가 발생했습니다.", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "백로그 생성 중 시스템 오류가 발생했습니다.");
        }
    });
    // 최근 실패/최근 evidence 조회 API (운영용)
    app.get("/v1/ops/reports/:gateKey/recent", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!auth)
                return;
            if (!(0, auth_1.isOps)(auth)) {
                (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/recent", code: "FORBIDDEN", messageKo: "운영자만 접근 가능합니다." });
                return (0, http_1.fail)(res, 403, "FORBIDDEN", "운영자만 접근 가능합니다.");
            }
            const days = Number(req.query.days) || 7;
            ...
            (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/recent", code: "INTERNAL", messageKo: "조회 중 시스템 오류가 발생했습니다.", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "조회 중 시스템 오류가 발생했습니다.");
        }
    });
    // Ops Console: Cloud Storage 백로그 다운로드 URL 발급
    app.get("/v1/ops/reports/:gateKey/ops-log/monthly/download-url", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!exists) {
                return (0, http_1.fail)(res, 404, "NOT_FOUND", `해당 월의 스토리지 백업 파일을 찾을 수 없습니다: ${objectPath}`);
            }
            ...
            (0, http_1.logError)({ endpoint: "monthly/download-url", code: "INTERNAL", messageKo: "다운로드 URL 생성 중 오류가 발생했습니다.", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "다운로드 URL 생성 중 오류가 발생했습니다.");
        }
    });
    // Ops Console: 특정 케이스 조회 API
    app.get("/v1/ops/reports/:gateKey/by-case", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!auth)
                return;
            if (!(0, auth_1.isOps)(auth)) {
                (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/by-case", code: "FORBIDDEN", messageKo: "운영자만 접근 가능합니다." });
                return (0, http_1.fail)(res, 403, "FORBIDDEN", "운영자만 접근 가능합니다.");
            }
            const caseId = req.query.caseId;
            if (!caseId || typeof caseId !== "string") {
                (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/by-case", code: "INVALID_ARGUMENT", messageKo: "caseId 파라미터가 필요합니다." });
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "caseId 파라미터가 필요합니다.");
            }
            ...
            (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/by-case", code: "INTERNAL", messageKo: "조회 중 시스템 오류가 발생했습니다.", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "조회 중 시스템 오류가 발생했습니다.");
        }
    });
    // 주간 리뷰용 내보내기 (markdown export)
    app.get("/v1/ops/reports/:gateKey/backlog.md", async (req, res) => {
        try {
            const gateKey = String(String(req.params.gateKey));
            if (!/^[a-z0-9-]+$/.test(gateKey)) {
                return (0, http_1.fail)(res, 400, "INVALID_ARGUMENT", "유효하지 않은 gateKey입니다.");
            ...
            if (!auth)
                return;
            if (!(0, auth_1.isOps)(auth)) {
                (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/backlog.md", code: "FORBIDDEN", messageKo: "운영자만 접근 가능합니다." });
                return (0, http_1.fail)(res, 403, "FORBIDDEN", "운영자만 접근 가능합니다.");
            }
            ...
            (0, http_1.logError)({ endpoint: "/v1/ops/reports/pilot-gate/backlog.md", code: "INTERNAL", messageKo: "내보내기 중 시스템 오류가 발생했습니다.", err });
            return (0, http_1.fail)(res, 500, "INTERNAL", "내보내기 중 시스템 오류가 발생했습니다.");
        }
    });
