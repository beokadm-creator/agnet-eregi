...
async function buildFilingSummaryDocx(adminApp, caseId) {
    ...
    if (!f) {
        ...
        children.push(new docx_1.Paragraph({ text: `접수번호: ${f.receiptNo ?? "-"}` }));
        children.push(new docx_1.Paragraph({ text: `관할: ${f.jurisdictionKo ?? "-"}` }));
        children.push(new docx_1.Paragraph({ text: `접수일: ${f.submittedDate ?? "-"}` }));
        if (f.memoKo)
...
async function buildSignedEvidenceReportDocx(adminApp, params) {
    ...
    for (const slotId of signedSlots) {
        ...
        rows.push([String(slotId), (0, casepack_1.getSlotTitleKo)(casePackId, String(slotId)) ?? String(slotId), statusKoOf(st)]);
    }
    if (rows.length === 1)
        rows.push(["-", "-", "-"]);
    const children = [];
